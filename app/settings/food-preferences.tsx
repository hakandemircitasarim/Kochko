  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: colors.background }} behavior="padding">
      <ScrollView style={{ flex: 1, backgroundColor: colors.background }} contentContainerStyle={{ padding: SPACING.md, paddingBottom: SPACING.xxl + insets.bottom }} keyboardShouldPersistTaps="handled">
        ...
        <Text style={{ ...TYPE.body, color: colors.textSecondary, marginTop: SPACING.xs, marginBottom: SPACING.lg }}>Sevdiğin ve sevmediğin yemekleri ekle. AI bunları dikkate alır.</Text>
        ...
        <Card>
            ...
            {(Object.keys(PREF_LABELS) as Pref[]).map(p => {
              ...
              return (
                <TouchableOpacity key={p} onPress={() => { haptics.tap(); setNewPref(p); }}
                  ...
                  <Text style={{ color: active ? getContrastColor(colors[PREF_COLOR_TOKENS[p]]) : colors.textSecondary, ...TYPE.caption }}>{PREF_LABELS[p]}</Text>
                </TouchableOpacity>
              );
            })}
          </View>
          <TouchableOpacity onPress={() => { haptics.tap(); setIsAllergen(!isAllergen); }}
            ...
            <Text style={{ color: colors.textSecondary, ...TYPE.body }}>Alerjen / İntolerans</Text>
          </TouchableOpacity>
