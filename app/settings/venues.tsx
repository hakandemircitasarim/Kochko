...
export default function VenuesScreen() {
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: colors.background }} contentContainerStyle={{ padding: SPACING.md, paddingBottom: SPACING.xxl + insets.bottom }}>
      ...
      <Text style={{ ...TYPE.body, color: colors.textSecondary, marginBottom: SPACING.md }}>Sık gittiğin mekanlar ve öğrenilen makro tahminleri.</Text>
      ...
      {loading ? (
        ...
        <Card>
          ...
          <View style={{ alignItems: 'center', paddingVertical: SPACING.xl, gap: SPACING.sm }}>
            ...
            <Text style={{ color: colors.text, ...TYPE.headline, textAlign: 'center' }}>Henüz kayıtlı mekan yok</Text>
            <Text style={{ color: colors.textSecondary, ...TYPE.body, textAlign: 'center' }}>Koçuna "Simit Sarayı'nda yedim" gibi yazdığında mekan otomatik öğrenilir.</Text>
          </View>
        </Card>
      ) : (
        venues.map(v => (
                  ...
                  {v.learned_items.map((item, i) => (
                    <View key={i} style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                      ...
                      <View style={{ flexDirection: 'row', gap: SPACING.sm }}>
                        <Text style={{ color: colors.text, ...TYPE.body }}>{item.calories} kcal</Text>
                        {item.confirmed && <Text style={{ color: colors.success, ...TYPE.caption }}>onaylı</Text>}
                      </View>
                    </View>
                  ))}
                </View>
