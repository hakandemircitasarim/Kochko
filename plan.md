...
- [x] **T1.20** Dinamik aktivite carpani: 2-4 haftalik veri sonrasi tahmini vs gercek kilo degisimini karsilastirarak carpani refine et
...
- [x] **T1.41** `goals.service.ts` tamamla: CRUD, deaktivasyon, otomatik faz gecisi
- [x] **T1.42** Hedef ekranini form ile tamamla: hedef tipi, hedef kilo, sure, oncelik, kisit modu
- [x] **T1.43** Hedef ilerlemesini haftalik rapora entegre et: Kullanici hedefe gore yolda mi?
...
- [x] **T2.1** Dashboard store'a haftalik butce hesaplama ekle: `getWeeklyStatus()` fonksiyonunu her gun fetch'inde cagir
- [x] **T2.2** `WeeklyBudgetWidget.tsx`'i gercek veriye bagla ve dashboard'da render et
- [x] **T2.3** Haftalik butce durumunu gunluk plana yansit: "Bu hafta X kcal marjin var" mesaji

### 2.2 Haftalik Rapor Tamamlama (Spec 8.2) - Item 13
**Dosyalar:** `app/reports/weekly.tsx`

- [x] **T2.4** Kilo trendi grafigi (react-native-chart-kit ile)
- [x] **T2.5** Uyum haritasi, en iyi/en kotu gun, sapma analizi gorsel
- [x] **T2.6** Guc progresyon ozeti (antrenman stili guc agirlikli ise)
- [x] **T2.7** AI ogrenme notu gorunumu
...
- [x] **T2.8** `templates.service.ts` CRUD dogrula: olusturma, silme, kullanim sayisi artirma, sablondan hizli kayit
- [x] **T2.9** Sablondan tek dokunusla ogun kaydi akisi: Chat'e veya dashboard'a "sablonlarim" butonu
- [x] **T2.10** AI'in 3+ kez tekrar eden ogunu tespit edip sablon onerme akisi (progressive disclosure ile)
- [x] **T2.11** Su hedefinin `calculateWaterTarget()` ile dinamik hesaplanmasi: antrenman gunu, mevsim, kafein
...
- [x] **T2.16** IF ayarlar ekranini dogrula: protokol secimi (16:8, 18:6, 20:4, ozel), saat girisi, profile kayit
- [x] **T2.17** Plan uretiminde IF penceresi zorunlulugu: Kod seviyesinde ogun onerilerinin pencere icinde oldugunu dogrula
- [x] **T2.18** Dashboard'da IF zamanlayici widget: Yeme penceresi acik/kapali, geri sayim
...
- [x] **T2.22** `ai-proactive/index.ts` mesajlarini push notification'a bagla
- [x] **T2.23** Bildirim izni UX akisi: Ilk kez stratejik zamanda iste, reddedilirse 3-5 gun sonra tekrar dene
...
- [x] **T2.25** MVD modunu uctan uca test et: "motivasyonum yok" -> 3 basit hedef, yumusak ton, baskilamama
...
- [x] **T2.26** 3 gun sessizlik: Hafif push notification (Layer 2 verisi kullanarak kisisellestir)
- [x] **T2.27** 7 gun sessizlik: Derin kissilestirme ile bildirim (streak, son basari referansi)
- [x] **T2.28** 14+ gun sessizlik: Email bildirimi (email gonderim altyapisi gerekli)
- [x] **T2.29** 6+ ay sonra geri donus: Re-onboarding akisi, profil guncelleme, TDEE yeniden hesaplama

### 2.9 Telefon Adim Sayaci (Spec 14.2) - Item 20
**Dosyalar:** `src/components/tracking/StepCounter.tsx`

- [x] **T2.30** `expo-sensors` veya Apple HealthKit / Google Fit entegrasyonu
- [x] **T2.31** `daily_metrics.steps` otomatik doldurma, `steps_source: 'phone'`

### 2.10 Fotograf ile Ogun Kaydi (Spec 3.1) - Item 21
**Dosyalar:** `app/(tabs)/chat.tsx`, `supabase/functions/ai-chat/index.ts`

- [x] **T2.32** Foto fallback: Dusuk guvenli analiz durumunda metin girisi isteme
- [x] **T2.33** Foto + metin combo: Chat'te thumbnail gosterimi iyilestirme
- [x] **T2.34** Bulanik/karanlik foto tespiti ve "metin olarak gir" yonlendirmesi

### 2.11 Porsiyon Kalibrasyonu (Spec 5.23) - Item 22
**Dosyalar:** `supabase/functions/shared/memory.ts`, `app/(tabs)/profile.tsx`

- [x] **T2.35** Onboarding'de AI porsiyon kalibrasyonu baslatma (ilk hafta icinde standart sorular)
- [x] **T2.36** Layer 2'deki porsiyon kalibrasyonlarini profil ekraninda gosterme/duzenleme
...
- [x] **T2.39** Uctan uca test: "pizza yesem ne olur?" -> kalan butce, alternatif, haftalik etki

### 2.14 Hizli Kurtarma Modu (Spec 5.2) - Item 25
- [x] **T2.40** Uctan uca test: "bugun cok yedim" -> empatik yanit, haftalik perspektif, mini kurtarma plani
...
- [x] **T2.42** Haftalik rapor veya plan uretiminde zorluk ayarlamasini tetikle

### 2.16 Beslenme Okuryazarligi (Spec 5.31) - Item 27
- [x] **T2.43** Layer 2'de `nutrition_literacy` seviyesinin dogru yazilip okundugunu dogrula
- [x] **T2.44** System prompt'un dusuk okuryazarlikta aciklama serpistigini test et

### 2.17 Progressive Disclosure (Spec 5.33) - Item 28
**Dosyalar:** `src/services/progressive-disclosure.service.ts`

- [x] **T2.45** `getFeaturesToIntroduce()` sonuclarini Layer 2 `features_introduced` dizisine yaz
- [x] **T2.46** `shared/memory.ts`'de Layer 2 ciktisina `features_introduced` ekle
- [x] **T2.47** AI'in zaten tanittigi ozellikleri tekrar tanitmamasini dogrula

### 2.18 Aliskanlik Koclugu (Spec 5.35) - Item 29
**Dosyalar:** `src/services/habits.service.ts`

- [x] **T2.48** `habits.service.ts` CRUD dogrula: aliskanlik onerisi, streak takibi, stacking mantigi
- [x] **T2.49** Layer 2'de `habit_progress` verisinin dogru yazildigini dogrula

### 2.19 AI Hedef Onerisi (Spec 6.4) - Item 30
**Dosyalar:** `supabase/functions/ai-proactive/index.ts`

- [x] **T2.50** 2+ haftalik veri sonrasi AI'in proaktif hedef onerisi gonderme mantigi
- [x] **T2.51** Ayda en fazla 1-2 oneri siniri
...
- [x] **T3.1** `weekly-plan.service.ts` tamamla: 7 gunluk plan uretimi, gun bazli degisiklik, alisveris listesi toplama
- [x] **T3.2** `weekly-menu.tsx` ekranini dogrula: 7 gunluk gorunum, ogun degistirme, alisveris listesi tab'i
- [x] **T3.3** Alisveris listesi uretimi: Malzemeleri benzersizlestir, miktarlari topla, kategori bazli gruplama

### 3.2 Tarif Kutuphanesi (Spec 7.7) - Item 32
**Dosyalar:** `src/services/recipes.service.ts`, `app/settings/recipes.tsx`

- [x] **T3.4** `recipes.service.ts` CRUD dogrula: kaydet, listele, filtrele, sil
- [x] **T3.5** `recipes.tsx` ekranini dogrula: kategori filtresi, malzeme listesi, pisirme talimatlari
- [x] **T3.6** Sohbette tarif kaydetme aksiyonu: AI tarif verdiginde "kaydet" butonu, `save_recipe` action tipi
- [x] **T3.7** Malzeme ikamesi: "Tavuk yerine ne koyabilirim?" akisi (AI prompt-driven)
- [x] **T3.8** "Elimde sunlar var" modu: Kullanicinin mevcut malzemelerinden tarif onerisi (AI prompt-driven)
...
- [x] **T3.10** Haftalik menu ekranina veya ayri ekrana meal prep bolumu ekle
- [x] **T3.11** Profildeki `meal_prep_active` ve `meal_prep_days` degerlerini kullan
...
- [x] **T3.14** Secilen stratejiyi plana uygulama akisi
...
- [x] **T3.16** Reverse diet: 2-4 haftalik kademeli kalori artisi plani
- [x] **T3.17** Tolerans bandi: Hedef kilodan +-1.5kg, asildiginda proaktif uyari
- [x] **T3.18** Bakim milestone'lari: 1 ay, 3 ay, 6 ay bakimda kalma basarilari
...
- [x] **T3.19** `multi-phase-goals.tsx` dogrula: Ardisik faz tanimlama, zaman cizelgesi gorunumu
- [x] **T3.20** Otomatik faz gecisi: Hedefe ulasildiginda veya sure doldugunda sonraki faza kademeli gecis
- [x] **T3.21** Her fazin ayri TDEE, makro ve tempo hesabi

### 3.7 Guc Antrenmani Progresyon (Spec 7.5) - Item 37
**Dosyalar:** `src/services/strength.service.ts`, `app/settings/strength.tsx`

- [x] **T3.22** `strength.service.ts` dogrula: `strength_sets` okuma, PR hesaplama, progresyon takibi
- [x] **T3.23** `strength.tsx` dogrula: Hareket listesi, 1RM gecmisi, PR rozeti, progresyon grafigi
- [x] **T3.24** AI chat'te guc set parse: "bench press 4x8 70kg" -> `strength_sets` tablosuna kayit
- [x] **T3.25** Deload onerisi: 4-6 haftalik yogun donem sonrasi otomatik deload hatirlat
...
- [x] **T3.27** Alkol kalorisinin gunluk toplama ve haftalik rapora ayri satirda yansimasi
- [x] **T3.28** Alkol-sapma korelasyonu: AI'in alkol sonrasi plansiz atistirma kalibini tespit etmesi

### 3.9 Lab Modulu (Spec 3.1) - Item 39
**Dosyalar:** `src/services/health.service.ts`, `app/settings/lab-values.tsx`

- [x] **T3.29** `lab-values.tsx` dogrula: Parametre ekleme, referans aralik, aralik disi uyari
- [x] **T3.30** AI proaktif tahlil onerisi: Onemli diyet degisikliginden 3 ay sonra, uzun kisitlama sonrasi

### 3.10 Donemsel Durum Yonetimi (Spec 9) - Item 40
**Dosyalar:** `src/services/periodic.service.ts`, `app/settings/periodic-state.tsx`

- [x] **T3.31** `periodic-state.tsx` dogrula: Ramazan, tatil, hastalik, hamilelik, seyahat vb. aktif etme
- [x] **T3.32** Donemsel durum hafizasi: AI gecmis donemleri Layer 2'ye yazsin, gelecek donemlere referans
- [x] **T3.33** Donem bitisinde kademeli normal plana gecis

### 3.11 Challenge Modulu (Spec 13.5) - Item 41
**Dosyalar:** `src/services/challenges.service.ts`, `app/settings/challenges.tsx`

- [x] **T3.34** `challenges.tsx` dogrula: Aktif challenge'lar, ilerleme barı, sistem challenge kutuphanesi
- [x] **T3.35** Challenge duraklatma: Donemsel durum devreye girdiginde otomatik duraklat
- [x] **T3.36** En fazla 2 aktif challenge siniri
- [x] **T3.37** Dashboard'da challenge ilerleme mini gostergesi

### 3.12 Mekan Hafizasi (Spec 2.1, 5.2) - Item 42
**Dosyalar:** `src/services/venues.service.ts`, `app/settings/venues.tsx`

- [x] **T3.38** `venues.tsx` dogrula: Ogrenilen mekanlar, yemek bilgileri, ziyaret sayisi
- [x] **T3.39** AI sohbette "Simit Sarayi'na gittim" denildiginde mekan hafizasini kullanma
- [x] **T3.40** Menu fotografı analizi: Restoran menusu fotolayip oneri alma
- [x] **T3.41** Sosyal yeme baskisi koclugu: "Is yemegindeyim" senaryosunda hasar minimizasyonu
...
- [x] **T3.43** Motivasyon dusus erken uyarisi: Streak kirilma riski, kayit sikligi dusme
- [x] **T3.44** Alkol-sapma tahmini: Cuma gunu proaktif strateji sunma
...
- [x] **T3.46** Kafein-uyku korelasyonu: 15:00 sonrasi kafein ile uyku kalitesi iliskisi
- [x] **T3.47** 400mg/gun uyarisi

### 3.15 Sohbet Gecmisi Arama (Spec 5.18) - Item 45
**Dosyalar:** `app/settings/chat-history.tsx`

- [x] **T3.48** `chat-history.tsx` dogrula: Oturum listesi, arama, konu etiketi, secerek/toplu silme
- [x] **T3.49** "Sohbet silinse bile kocun ogrendikleri korunur" uyarisi
...
- [x] **T4.3** Speech-to-text sonucunu normal metin mesaji olarak isle
...
- [x] **T4.7** Yuz bulaniklastirma secenegi (paylasim oncesi)
...
- [x] **T4.8** PDF formatinda profesyonel rapor uretimi
- [x] **T4.9** Kullanicinin raporda hangi verilerin yer alacagini secmesi
- [x] **T4.10** CSV ham veri export secenegi

### 4.4 Baska Uygulamadan Veri Import (Spec 14.4) - Item 49
**Dosyalar:** `src/services/import.service.ts`, `app/settings/data-import.tsx`

- [x] **T4.11** MyFitnessPal CSV format destegi
- [x] **T4.12** FatSecret format destegi
- [x] **T4.13** Import edilen verilerin gecmis kayit olarak islenmesi

### 4.5 Saat Dilimi / Seyahat Yonetimi (Spec 2.5) - Item 50
**Dosyalar:** `src/lib/day-boundary.ts`, `src/stores/profile.store.ts`

- [x] **T4.14** Otomatik timezone tespiti: Telefon timezone'u degistiginde algilanip profile yazilsin
- [x] **T4.15** IF penceresi + timezone cakismasi: 6+ saat farkta pencere esnetme
...
- [x] **T4.24** Hizli secim butonlari: "Aksam ne yiyelim?" sorusuna 2-3 secenek butonu
- [x] **T4.25** Onay butonlari: Plan degisikligi onerildikginde "Onayla / Reddet"
...
- [x] **T4.34** AI gorev modunu ve guardrail tetiklenmelerini goster
...
- [x] **T4.41** Ayarlarda tema secimi: Sistemi takip et / Her zaman acik / Her zaman koyu
- [x] **T4.42** `accessibility.ts` helper'larini uygula: Min 44x44px dokunma alanlari, screen reader etiketleri
- [x] **T4.43** WCAG AA kontrast orani kontrolu
...
- [x] **T4.46** Ag durumu izleme: UI'da offline gostergesi
- [x] **T4.47** Barkod offline cache: Onceden taranan urunler yerel calissin

### 4.16 Aylik ve Tum Zamanlar Raporu (Spec 8.3-8.4)
**Dosyalar:** `supabase/functions/ai-report/index.ts`

- [x] **T4.48** Aylik rapor uretimi: 4 haftalik raporlarin birlestirilmesi, trend analizi
- [x] **T4.49** Yeni `app/reports/monthly.tsx` ekrani
- [x] **T4.50** Tum zamanlar raporu: Baslangictan bugune toplam ilerleme, en uzun streak, km taslari
...
- [x] **T4.53** Veri silme akisi: 30 gunluk yumusak silme (T1.8 ile baglanli)
- [x] **T4.54** Katman 2 seffafligi: "Benim hakkimda ne biliyorsun?" ekrani, duzeltme/silme hakki
- [x] **T4.55** Ogun fotograflari retention: Parse sonrasi 24 saat icinde sunucudan silme

### 4.18 i18n Altyapisi (Spec 20.2)
**Dosyalar:** `src/lib/i18n.ts`

- [x] **T4.56** `i18n.ts` tamamla: Tum hard-coded Turkce stringleri cikart
- [x] **T4.57** Ingilizce dil dosyasi hazirla
- [x] **T4.58** Dil secimi ayari ekle
