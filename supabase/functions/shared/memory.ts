...
Diyet modu: ${p.diet_mode ?? 'standard'}${p.dietary_restriction ? ` | KISITLAMA: ${p.dietary_restriction}` : ''}${p.if_active ? ` | IF: ${p.if_window} (${p.if_eating_start}-${p.if_eating_end})` : ''}
Koc tonu: ${p.coach_tone ?? 'balanced'}
${(p.digestive_issues || p.hormone_conditions) ? `SAGLIK (planda dikkate al): ${[p.digestive_issues && `Sindirim: ${p.digestive_issues}`, p.hormone_conditions && `Hormonal: ${p.hormone_conditions}`].filter(Boolean).join(' | ')}` : ''}
${(p.training_experience || p.preferred_exercises || p.disliked_exercises || p.available_training_times) ? `ANTRENMAN: ${[p.training_experience && `Deneyim: ${p.training_experience}`, p.preferred_exercises && `Sevdigi: ${p.preferred_exercises}`, p.disliked_exercises && `SEVMEDIGI: ${p.disliked_exercises}`, p.available_training_times && `Saatler: ${p.available_training_times}`].filter(Boolean).join(' | ')}` : ''}
${(p.waist_cm || p.hip_cm || p.body_fat_pct || p.step_target) ? `OLCU/HEDEF: ${[p.waist_cm && `Bel: ${p.waist_cm}cm`, p.hip_cm && `Kalca: ${p.hip_cm}cm`, (p.waist_cm && p.hip_cm) && `Bel/Kalca oran: ${((p.waist_cm as number)/(p.hip_cm as number)).toFixed(2)}`, p.body_fat_pct && `Yag: %${p.body_fat_pct}`, p.step_target && `Adim hedefi: ${p.step_target}`].filter(Boolean).join(' | ')}` : ''}
